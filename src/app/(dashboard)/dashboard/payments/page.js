...
import { getPaymentStats, getPaymentHistory, createStripeDashboardLink } from '@/app/actions'; // Import Server Actions
import { ArrowTopRightOnSquareIcon } from '@heroicons/react/24/outline';

const formatCurrency = (cents, currency = 'USD') => new Intl.NumberFormat('en-US', { style: 'currency', currency: currency.toUpperCase() }).format(cents / 100);
const formatDate = (dateString) => new Date(dateString).toLocaleString(undefined, { year: 'numeric', month: 'long', day: 'numeric' });
    ...
    useEffect(() => {
        startTransition(async () => {
            ...
            if (statsResult.success) {
                ...
                setError(statsResult.message || "Failed to load stats.");
            }
            if (historyResult.success) {
                setHistory(historyResult.data);
            } else {
                setError(prev => `${prev || ''} ${historyResult.message || "Failed to load history."}`);
            }
                        ...
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                {history.length > 0 ? (
                                    ...
                                    <tr><td colSpan="3" className="px-6 py-10 text-center text-gray-500 dark:text-gray-400">No gifts yet.</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
