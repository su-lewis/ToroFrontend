...
async function getPublicProfileData(username) { /* ... same as before ... */ }

export default async function PublicProfilePage({ params, searchParams }) {
  const username = params.username;
  const profileData = await getPublic-profileData(username);
    ...
    <div style={pageStyle} className="min-h-screen transition-colors duration-500">
      <div className="container mx-auto max-w-3xl flex flex-col items-center pb-12">
        ...
        <div className="w-full max-w-2xl px-6 md:px-8 relative z-10 -mt-16 md:-mt-20">
          ...
          {paymentCancelled && (
            <div role="alert" className="mb-6 p-3 bg-yellow-100 border ...">
              ...
              {links.length > 0 ? (
                links.map((link) => (
                  <a key={link.id} href={link.url.startsWith('http') ? link.url : `//${link.url}`} target="_blank" rel="noopener noreferrer nofollow"
                    className="block w-full text-center font-semibold py-3 px-5 rounded-lg text-lg shadow-md hover:shadow-lg transition-all bg-black bg-opacity-10 hover:bg-opacity-20 border border-white border-opacity-20 backdrop-blur-sm"
