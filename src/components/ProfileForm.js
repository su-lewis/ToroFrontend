...
export default function ProfileForm({ initialData, serverError }) {
  ...
  return (
    // The form container itself will respect dark mode
    <form ref={formRef} onSubmit={handleFormSubmit} className="space-y-8 max-w-3xl mx-auto bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg">
        ...
        <div>
          ...
          <div className="w-full aspect-[3/1] rounded-lg bg-gray-100 dark:bg-gray-700 relative flex items-center justify-center border-2 border-dashed border-gray-300 dark:border-gray-600">
            ...
            <span className="z-10 p-2 bg-white/70 dark:bg-black/50 text-gray-600 dark:text-gray-200 rounded-md pointer-events-none">Click or drag to upload banner</span>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <input type="text" name="username" id="username" defaultValue={initialData?.username || ''} required 
                   className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm text-black bg-white placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500"/>
          </div>
          <div>
            <label htmlFor="displayName" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Display Name</label>
            {/* --- FIX: REMOVED dark:... classes to keep it light-themed --- */}
            <input type="text" name="displayName" id="displayName" defaultValue={initialData?.displayName || ''} 
                   className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm text-black bg-white placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500"/>
          </div>
          <div>
            <label htmlFor="bio" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Bio</label>
            {/* --- FIX: REMOVED dark:... classes to keep it light-themed --- */}
            <textarea name="bio" id="bio" rows="4" defaultValue={initialData?.bio || ''} 
                      className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm text-black bg-white placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500"></textarea>
          </div>
        </div>
